// ─── Palette generator — the single source of truth for ideafeed's themes ────
// Ported from board-room, which solved this properly: 20 palettes as a DATA
// TABLE rather than 400 hand-maintained hex values. Emits both
// src/design/themes.css (what the browser reads) and src/design/palettes.ts
// (labels and swatches for the picker) from one table, because maintaining
// those separately guarantees they drift, and a drifted swatch means the
// picker lies about what you are choosing.
//
// Only TEN tokens are authored per palette per mode. Everything else in the
// app derives from them through color-mix — lines, the accent wash, the
// glass header — so overriding --text carries the whole ink ladder with it.
// That is what makes twenty themes a table instead of a rewrite.
//
// Semantic colours (danger, warn) are NOT per-palette. They mean something
// fixed — a rejected skill is red in all twenty — and re-tinting them per
// theme would break that contract for the sake of decoration.
//
// Every generated value is contrast-checked against WCAG AA before it is
// written; the script exits non-zero rather than emitting a failing palette.
...
const PALETTES = THEMES.map(t => ({ ...t, light: build(t, "light"), dark: build(t, "dark") }));
...
if (isMain) {
  ...
  // ─── emit ────────────────────────────────────────────────────────────────────
  // The generator's vocabulary (surface / ink / sub / faint) is board-room's.
  // ideafeed's stylesheet already speaks its own — panel / text / text-dim /
  // muted — across 1700 lines, so the mapping happens HERE rather than by
  // renaming every rule in the app.
  const MAP = {
    bg: "bg",
    surface: "panel",
    "surface-2": "panel-2",
  ...
  const banner = `/* GENERATED by scripts/gen-themes.mjs — do not edit by hand.
   Run \`npm run themes\` after changing the table in that file.

   ${PALETTES.length} palettes x light/dark. Each authors only the ten tokens the rest
   of the system derives from; --line, --accent-soft, --bg-soft and the rest are
   color-mix over these, so they follow for free. Semantic colours (danger,
   warn) stay in styles.css: they encode meaning, not taste. */
`;

  let css = banner + `
/* Derived from the ten authored tokens, so every palette gets these free.
   Imported after the :root defaults in styles.css and matching their
   specificity, so it wins; the [data-palette][data-theme] blocks below are
   more specific still. */
[data-palette] {
  ...
  /* The one place --accent-2 is used is a secondary link; following the accent
     keeps it inside the palette instead of introducing a 21st colour. */
  --accent-2: var(--accent-hi);
}
[data-palette][data-theme="light"] {
  --bg-soft: color-mix(in srgb, var(--bg) 92%, var(--text));
  --shadow: 0 12px 32px color-mix(in srgb, var(--text) 12%, transparent);
}
[data-palette][data-theme="dark"] {
  --bg-soft: color-mix(in srgb, var(--bg) 94%, var(--text));
...
`;
  ...
  for (const p of PALETTES) {
    for (const mode of ["light", "dark"]) {
      css += `\n/* ${p.label} · ${mode} — ${p.blurb} */\n`;
      css += `[data-palette="${p.key}"][data-theme="${mode}"] {\n`;
      for (const [from, to] of Object.entries(MAP)) css += `  --${to}: ${p[mode][from]};\n`;
  ...
  const ts = `// GENERATED by scripts/gen-themes.mjs — do not edit by hand.
...
${PALETTES.map(p => `  {
    ...
    light: { bg: '${p.light.bg}', panel: '${p.light.surface}', accent: '${p.light.accent}', text: '${p.light.ink}' },
    dark: { bg: '${p.dark.bg}', panel: '${p.dark.surface}', accent: '${p.dark.accent}', text: '${p.dark.ink}' },
  },`).join("\n")}
];
...
`;
  writeFileSync("src/design/palettes.ts", ts);

  // The pre-paint script in index.html needs the grounds BEFORE any CSS exists
  // (the stylesheet is emitted after it), so they're inlined there rather than
  // read from the cascade. Generated here so there is still one source of truth
  // — a hand-copied map is exactly the thing that goes stale and paints the
  // wrong colour behind the iOS status bar for one frame on every cold start.
  const grounds = "      var G = " + JSON.stringify(
    Object.fromEntries(PALETTES.map(p => [p.key, [p.light.bg, p.dark.bg]]))
  ) + ";";
  ...
  console.log(`  wrote src/design/palettes.ts`);
